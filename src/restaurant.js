...
function newOrder(pedido, obj) {
  obj.consumption.push(pedido);
  const menu = obj.fetchMenu();

  if (('food' in menu) && (pedido in menu.food)) {
    obj.conta += menu.food[pedido];
  } else if (('drink' in menu) && (pedido in menu.drink)) {
    obj.conta += menu.drink[pedido];
  }
}
...
const createMenu = (menu) => { 
  const obj = {
    ...
    consumption: [],
    conta: 0,
    order: (pedido) => newOrder(pedido, obj),
    pay: (pedido) => obj.conta,
  };
