...
describe('10 - Implemente a função `createMenu`, bem como seus casos de teste', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    const objetoTeste = { 
      food: {'coxinha': 3.9, 'sopa': 9.9, 'sashimi': 10}, 
      drink: {'agua': 3.9, 'cerveja': 6.9} 
    };
    objetoRetornado = createMenu(objetoTeste);
    const objetoFetched = objetoRetornado.fetchMenu()

    // TESTE 1: Verifique se função `createMenu()` retorna um objeto que possui a chave `fetchMenu`, a qual tem como valor uma função.
    expect(objetoRetornado.hasOwnProperty('fetchMenu')).toEqual(true);

    // TESTE 2: Verifique se 'objetoRetornado.fetchMenu()' retorna um objeto cujas chaves são somente `food` e `drink`,
    expect(objetoFetched.hasOwnProperty('food')).toEqual(true);
    expect(objetoFetched.hasOwnProperty('drink')).toEqual(true);
    expect(Object.keys(objetoFetched).length).toEqual(2);
    ...
    expect(objetoFetched).toEqual(objetoTeste);
    ...
    // TESTE 5: Verifique se, ao chamar uma função associada à chave `order` no objeto retornado, passando uma string como parâmetro (como `objetoRetornado.order('coxinha')`), tal string é adicionada ao array retornado em `objetoRetornado.consumption`.
    objetoRetornado.order('coxinha');
    expect(objetoRetornado.consumption[0]).toEqual('coxinha');
    ...
    expect(objetoRetornado.consumption).toEqual(['coxinha', 'agua', 'cerveja', 'sashimi', 'sopa'])

    // TESTE 7: Verifique se a função `order` aceita que pedidos repetidos sejam acrescidos a `consumption`.
    objetoRetornado.order('agua');
    expect(objetoRetornado.consumption).toEqual(['coxinha', 'agua', 'cerveja', 'sashimi', 'sopa', 'agua']);
    ...
    expect(objetoRetornado.pay()).toEqual(38.5)
